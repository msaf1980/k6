...
func TestParseThresholdCondition(t *testing.T) {
	...
	tests := []struct {
		...
		{"valid > operator", args{"med===100"}, &thresholdCondition{"med", "===", 100}, false},
		{"valid > operator", args{"med!=100"}, &thresholdCondition{"med", "!=", 100}, false},
		{"threshold expressions spaces are ignored", args{"count  < 200"}, &thresholdCondition{"count", "<", 200}, false},
		{"threshold expressions ending newlines are ignored", args{"count<200\n"}, &thresholdCondition{"count", "<", 200}, false},
		{"threshold expressions newlines are invalid", args{"count\n<200"}, nil, true},
		{"threshold expressions tabs are invalid", args{"count\t<\t\t\t200"}, nil, true},
